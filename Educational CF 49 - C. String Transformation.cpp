...
{
    //input;
    string str,main="abcdefghijklmnopqrstuvwxyz",newStr="";
    cin >> str;
    int matched = 0;
    if(main.sz<26){
        cout << -1;
    }else{
        for(int i=0,j=0;i<str.sz;i++){
            //dbg(matched,str[i],main[j],newStr);
            //if(matched==0 || matched>26) newStr+=str[i];
            if(str[i]<=main[j]){
                matched++;
                newStr+=main[j];
                j++;
