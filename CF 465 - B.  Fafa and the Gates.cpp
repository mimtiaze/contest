...
int main()
{
    ...
    cin >> str;
    int coin = 0,gate=0;
    int insideX,insideY,inside;
    insideX=insideY=inside=0;
    if(str[0]=='R') inside=1;
    FOR(n){
        if(str[i]=='U'){
            y++;
        ...
        if(x==y){
            gate=1;
            insideX=insideY=x;
        }else if(gate==1 && ((x>insideX && y==insideY && !inside) || (x==insideX && y>insideY && inside))){
        ...
        //dbg(x,y,inside,insideX,insideY,gate);
