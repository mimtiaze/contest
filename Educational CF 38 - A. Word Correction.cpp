...
template <typename T>inline bool isVowel(T ch){ ch=toupper(ch); if(ch=='A'||ch=='U'||ch=='I'||ch=='O'||ch=='E'||ch=='Y') return true; return false;}
...
int main()
{
    ...
    string ans="";
    FOR(str.lg){
        if(isVowel(str[i]) && toggle==0){
            toggle=1;
            ans+=str[i];
        }else if(isVowel(str[i]) && toggle==1){
            continue;
        }else{
            toggle=0;
            ans+=str[i];
